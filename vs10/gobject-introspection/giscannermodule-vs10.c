...
#define NEW_CLASS(ctype, name, cname, count)	      \
static const PyMethodDef _Py##cname##_methods[count];    \
...
NEW_CLASS (PyGISourceSymbol, "SourceSymbol", GISourceSymbol, 11);
...
static const PyGetSetDef _PyGISourceSymbol_getsets[11] = {
  /* int ref_count; */
...
static const PyGetSetDef _PyGISourceType_getsets[9] = {
				...
				PyObject          *args)
{
   ...
   */
  {
    ...
    msvcr90 = GetModuleHandle ("msvcr90.dll");
    if (!msvcr90)
      {
	g_print ("No msvcr90.dll loaded.\n");
	return NULL;
      }

    p__get_osfhandle = (int(*)(int))GetProcAddress (msvcr90, "_get_osfhandle");
    if (!p__get_osfhandle)
      {
	g_print ("No _get_osfhandle found in msvcr90.dll.\n");
	return NULL;
      }
    ...
    if (!p__get_osfhandle)
      {
	g_print ("Could not get OS handle from msvcr90 fd.\n");
	return NULL;
      }
...
static const PyMethodDef _PyGISourceScanner_methods[8] = {
