...
static void
gtk_entry_update_state (GtkEntry *entry)
{
  GtkEntryPrivate *priv = entry->priv;
  GtkStateFlags new_state;
  gboolean depressed;

  new_state = gtk_widget_get_state_flags (GTK_WIDGET (entry)) &
    ~(GTK_STATE_FLAG_PRELIGHT);

  if (priv->in_entry)
    new_state |= GTK_STATE_FLAG_PRELIGHT;

  gtk_widget_set_state_flags (GTK_WIDGET (entry), new_state, TRUE);
}
