/*
 * Copyright (C) 2001 - 2002 Carlos Perell?Marín <carlos@gnome-db.org>
				     ...
				     G_GNUC_UNUSED guint *task_id, GdaServerProviderAsyncCallback async_cb, G_GNUC_UNUSED gpointer cb_data)
{
	...
	if (! is_virtual) {
		...
		else {			/* try first with the file extension */
			gchar *tmp, *f1, *f2;
...
make_last_inserted_set (GdaConnection *cnc, GdaStatement *stmt, sqlite3_int64 last_id)
{
	...
	if (!statement) {
		...
		sql_statement = gda_sql_statement_new (GDA_SQL_STATEMENT_SELECT);
